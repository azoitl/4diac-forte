...
class EmbrickBusHandler : public forte::core::io::IODeviceMultiController {
  ...
  public:
    ...
    enum Command {
       ...
       * If a slave, for example, has 8 digital outputs, the master sends 1 Byte (1 Bit for each digital output) to set
       * the slave state. Likewise if a slave reads 8 analog values, the slave sends 16 Bytes (2 Bytes for each value)
       * to the master. The structure of the process image is specified in the emBRICK products manual. The Data command
       * should be performed at least 20 times per second.
       */
      Data = 10,
    };
    ...
    struct Config : forte::core::io::IODeviceController::Config {
        ...
        unsigned long mBusLoopSpeed; //!< Sets the maximal SPI speed for the brickBUS during the runtime updates of the
                                     //!< slaves. The default value is 700000 Hz.
    };
