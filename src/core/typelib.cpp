...
CFunctionBlock *CTypeLib::createGenericFB(CStringDictionary::TStringId paInstanceNameId, CStringDictionary::TStringId paFBTypeId, forte::core::CFBContainer &paContainer) {
  ...
  const char *underScore = getFirstNonTypeNameUnderscorePos(acTypeBuf);
  ...
  ptrdiff_t typeNameLen = underScore - acTypeBuf;
  std::string genFBName;
  genFBName.reserve(4 + typeNameLen);
  genFBName += "GEN_";
  genFBName.append(acTypeBuf, typeNameLen);

  CTypeEntry *typeEntry = findType(CStringDictionary::getInstance().getId(genFBName.c_str()), mFBLibStart);
  ...
  if(newFB == nullptr) { // we could not create the requested object
    ...
    if(!newFB->configureFB(acTypeBuf)) {
      deleteFB (newFB);
      return nullptr;
    }
  }
